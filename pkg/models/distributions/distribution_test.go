...
func TestCheckTypedRelationKey(t *testing.T) {
	assert := assert.New(t)
	for _, c := range []struct {
		...
		{colType: qdb.ColumnTypeInteger, dsType: qdb.ColumnTypeUinteger},
		{colType: qdb.ColumnTypeUinteger, dsType: qdb.ColumnTypeUinteger},
		{colType: qdb.ColumnTypeVarchar, dsType: qdb.ColumnTypeVarcharHashed},
		{colType: qdb.ColumnTypeUUID, dsType: qdb.ColumnTypeUUIDHashed},
		// Hash function support is checked when the relation is attached.
		{colType: qdb.ColumnTypeInteger, dsType: qdb.ColumnTypeInteger},
		{
			colType: qdb.ColumnTypeVarchar,
			dsType:  qdb.ColumnTypeUinteger,
			err: spqrerror.New(spqrerror.SPQR_INVALID_REQUEST,
				"column type varchar of relation r does not match type uinteger of distribution ds"),
		},
	} {
		err := distributions.CheckTypedRelationKey(
			distributions.NewDistribution("ds", []string{c.dsType}),
			&spqrparser.DistributedRelation{
				Relation: &rfqn.RelationFQN{RelationName: "r"},
				DistributionKey: []spqrparser.DistributionKeyEntry{
					{Column: "i", ColumnType: c.colType, HashFunction: "murmur"},
				},
			})
		if c.err != nil {
			assert.EqualError(err, c.err.Error(), "column type %s on %s", c.colType, c.dsType)
		} else {
			assert.NoError(err, "column type %s on %s", c.colType, c.dsType)
		}
	}
}
